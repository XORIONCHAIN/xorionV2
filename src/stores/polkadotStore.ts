...
const formatTxor = (rawBalance: string, decimals = 18): string => {
  ...
  const fraction = bn.mod(divisor).toString().padStart(decimals, '0').slice(0, 4).replace(/0+$/, '');
...
async function fetchEnhancedTransactionData(api: ApiPromise) {
  ...
  const latestBlockNumber = (await api.rpc.chain.getHeader(finalizedHead)).number.toNumber();

  const blockNumbers = Array.from({ length: 10 }, (_, i) => latestBlockNumber - i).filter(n => n >= 0);

  // 1. Get all block hashes in parallel
  const blockHashes = await Promise.all(
    blockNumbers.map(number => api.rpc.chain.getBlockHash(number))
  );

  // 2. Get all blocks and events in parallel
  const blocksAndEvents = await Promise.all(
    blockHashes.map(hash => Promise.all([
  ...
  for (let i = 0; i < blockNumbers.length; i++) {
    ...
    try {
      ...
      block.block.extrinsics.forEach((extrinsic, index) => {
        ...
        const transferInfo = detectTransferFromExtrinsic(extrinsic, extrinsicEvents, api);
        ...
        const transaction: Transaction = {
          ...
          transferAmount: transferInfo.amount ? formatTxor(transferInfo.amount) : undefined,
  ...
  return {
    transactions: transactions.slice(0, 100).sort((a, b) => b.blockNumber - a.blockNumber),
    blocks,
    lastUpdated: Date.now()
  };
}
...
function detectTransferFromExtrinsic(extrinsic: any, events: any[], api: ApiPromise) {
  ...
  const transferMethods = [
    'transfer', 'transferKeepAlive', 'transferAll', 'forceTransfer',
    'transferAllowDeath', 'transferWithFee'
  ];
  
  if ((section === 'balances' || section === 'currencies' || section === 'tokens' || section === 'assets') &&
      transferMethods.some(tm => method.toLowerCase().includes(tm.toLowerCase()))) {
  ...
  const stakingMethods = ['bond', 'bondExtra', 'unbond', 'withdrawUnbonded', 'nominate', 'chill'];
...
async function findTransactionByHash(api: ApiPromise, hash: string): Promise<TransactionDetails | null> {
  try {
    // Normalize the hash input
    const normalizedHash = hash.trim();
    const searchHash = normalizedHash.startsWith('0x') ? normalizedHash : `0x${normalizedHash}`;
    
    console.log(`🔍 Searching for transaction hash: ${searchHash}`);
    
    // Add timeout to prevent hanging
    const searchPromise = performTransactionSearch(api, searchHash, normalizedHash);
    const timeoutPromise = new Promise<null>((_, reject) => 
      setTimeout(() => reject(new Error('Search timeout - transaction not found in recent blocks')), 30000)
    );
...
async function performTransactionSearch(api: ApiPromise, searchHash: string, normalizedHash: string): Promise<TransactionDetails | null> {
